...
void fu(const int Nr, const int Ntheta, const int Nphi, double* r, double* w, double* wt, double rs, double dr2, double dr, double dtheta,
  ...
  if (i>0 and i<(Nr-1) and j>0 and j<(Ntheta-1) and k>0 and k<(Nphi-1)){
    ...
    f[ijk]=wt[ijk];
    ft[ijk]=pow(t/(t-rs), 2.0)/dr2*(w[ijk-Ntheta*Nphi]-2.0*w[ijk]+w[ijk+Ntheta*Nphi])
    -1.0/(t*(t-rs))/tan(theta[j])/(2*dtheta)*(w[ijk+Nphi]-w[ijk-Nphi])
    -1.0/(t*(t-rs))/dtheta2*(w[ijk-Nphi]-2*w[ijk]+w[ijk+Nphi])
    -1.0/(t*(t-rs))/pow(sin(theta[j]),2.0)/dphi2*(w[ijk-1]-2.0*w[ijk]+w[ijk+1])+mu2*w[ijk]-lamb*pow(w[ijk], 3.0)-(2.0*t-rs)/(t*(t-rs))*wt[ijk];
  }
}
''', 'fu')
